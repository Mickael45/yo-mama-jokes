...
import { AWFUL_MAMA_JOKE_CATEGORY, BALD_MAMA_JOKE_CATEGORY, CLUMSY_MAMA_JOKE_CATEGORY, DIRTY_MAMA_JOKE_CATEGORY, DUMB_MAMA_JOKE_CATEGORY, ENTITLED_MAMA_JOKE_CATEGORY, EVIL_MAMA_JOKE_CATEGORY, FAT_MAMA_JOKE_CATEGORY, GREEDY_MAMA_JOKE_CATEGORY, HAIRY_MAMA_JOKE_CATEGORY, LAZY_MAMA_JOKE_CATEGORY, LOUD_MAMA_JOKE_CATEGORY, NASTY_MAMA_JOKE_CATEGORY, OLD_MAMA_JOKE_CATEGORY, OTHER_MAMA_JOKE_CATEGORY, POOR_MAMA_JOKE_CATEGORY, SCARY_MAMA_JOKE_CATEGORY, SHORT_MAMA_JOKE_CATEGORY, SKINNY_MAMA_JOKE_CATEGORY, TALL_MAMA_JOKE_CATEGORY, UGLY_MAMA_JOKE_CATEGORY } from '@/constants';
...
function seededRandom(seed: number): () => number {
  ...
  function getDailyRandomJokes(): CategoryJoke[] {
    ...
    // Select 5 random subarrays
    while (selectedCategorizedJokes.length < 15) {
      const randomIndex = Math.floor(random() * categorizedJokes.length);
      const selectedArray = categorizedJokes[randomIndex];

      if (!selectedCategorizedJokes.map(categorizedJoke => categorizedJoke).includes(selectedArray.category)) {
    ...
    selectedCategorizedJokes.forEach((selectedCategorizedJoke) => {
      if (selectedCategorizedJoke.jokes.length > 0) {
        const randomIndex = Math.floor(random() * selectedCategorizedJoke.jokes.length);
        result.push({ category: selectedCategorizedJoke.category, joke: selectedCategorizedJoke.jokes[randomIndex] });
      }
    });
